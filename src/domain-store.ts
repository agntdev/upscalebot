...
// The membership group is part of this bot's published service. Owner controls
// may replace these values, but a fresh deployment must be able to verify the
// supplied group immediately.
export const DEFAULT_GROUP_CHAT_ID = "1004301841447";
export const DEFAULT_GROUP_INVITE_LINK = "https://t.me/nastymovielounge";
...
export async function settings(ctx: StoreCtx): Promise<Settings> { const d = await ready(ctx); if (!d) return { groupChatId: DEFAULT_GROUP_CHAT_ID, groupInviteLink: DEFAULT_GROUP_INVITE_LINK, retentionDays: 30, standardLabel: "Standard — 2×", proLabel: "Pro — 4×" }; const rows = (await d.prepare("SELECT key, value FROM iu_settings").all<{key:string;value:string}>()).results; const value = (key: string) => rows.find((row) => row.key === key)?.value; return { groupChatId: value("group_chat_id") ?? DEFAULT_GROUP_CHAT_ID, groupInviteLink: value("group_invite_link") ?? DEFAULT_GROUP_INVITE_LINK, retentionDays: Number(value("retention_days") ?? 30), standardLabel: value("standard_label") ?? "Standard — 2×", proLabel: value("pro_label") ?? "Pro — 4×" }; }
